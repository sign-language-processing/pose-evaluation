...
def main(
    dataset_df_files: Annotated[
        list[Path],
        typer.Argument(help="List of dataset csvs, with columns POSE_FILE_PATH, SPLIT, and VIDEO_ID"),
    ],
...
# # top 20 dtw metrics from round 4 1206 metrics
# conda activate /opt/home/cleong/envs/pose_eval_src && cd /opt/home/cleong/projects/pose-evaluation && python pose_evaluation/evaluation/load_splits_and_run_metrics.py dataset_dfs/asl-citizen.csv --splits "train,test" --full --max-workers 64 --batch-size 100 --specific-metrics-csv metric_results_round_4_pruned_to_match_embeddings/5_21_score_analysis_1206metrics_169glosses/combined_metric_stats_dtw_hands_defaultdist0_20_metrics.csv --full-intersplit --out metric_results_full_matrix/ 2>&1|tee out/full_matrix$(date +%s).txt
# top 10 APE metrics from round 4 1206 metrics
# conda activate /opt/home/cleong/envs/pose_eval_src && cd /opt/home/cleong/projects/pose-evaluation && python pose_evaluation/evaluation/load_splits_and_run_metrics.py dataset_dfs/asl-citizen.csv --splits "train,test" --full --max-workers 64 --batch-size 100 --specific-metrics-csv metric_results_round_4_pruned_to_match_embeddings/5_21_score_analysis_1206metrics_169glosses/combined_metric_stats_top_10_ape.csv --full-intersplit --out metric_results_full_matrix/ --full-matrix-max-hyp 1000 2>&1|tee out/full_matrix$(date +%s).txt
