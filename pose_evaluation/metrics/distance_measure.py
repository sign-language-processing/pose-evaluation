...
class AggregatePointWiseThenAggregateTrajectorywiseDistanceSignature(
    DistanceMeasureSignature
):
...
class AggregatePointWiseThenAggregateTrajectorywiseDistance(DistanceMeasure):
    ...
    def __init__(
        ...
        trajectorywise_aggregator: DistanceAggregator,
    ) -> None:
        super().__init__()
    ...
    def pointwise_distance(
        self, hyp_point: MaskedArray, ref_point: MaskedArray
    ) -> float:
    ...
    def get_trajectory_pair_distances(
        self, hyp_trajectories: MaskedArray, ref_trajectories: MaskedArray
    ) -> Iterable[float]:
        return [
            self.get_trajectory_pair_distance(hyp_traj, ref_traj)
            for hyp_traj, ref_traj in zip(hyp_trajectories, ref_trajectories)
        ]

    def aggregate_pointwise_distances(
        self, pointwise_distances: Iterable[float]
    ) -> float:
    ...
    def aggregate_trajectory_distances(
        self, trajectory_distances: Iterable[float]
    ) -> float:
    ...
    def get_distance(self, hyp_data: MaskedArray, ref_data: MaskedArray) -> float:
        keypoint_trajectory_distances = self.get_trajectory_pair_distances(
            hyp_data, ref_data
        )
