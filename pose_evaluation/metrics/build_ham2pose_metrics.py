from pose_evaluation.metrics.distance_measure import AggregatePointWiseThenAggregateTrajectorywiseDistance, PowerDistance
from pose_evaluation.metrics.distance_metric import DistanceMetric
from pose_evaluation.metrics.ham2pose_distances import Ham2PoseMSEDistance, Ham2PoseMaskedEuclideanDistance, Ham2PoseAPEDistance
...
from pose_evaluation.metrics.pose_processors import PadOrTruncateByReferencePosesProcessor, get_standard_pose_processors

if __name__ == "__main__":

    metrics = []
    MJEMetric = MeanJointErrorMetric() #automatically sets distance measure, zero-padding.
    metrics.append(MJEMetric)    
    
    Ham2Pose_DTW_MJE_Metric = DTWMetric(name="DTW_MJE", 
                               distance_measure=PowerDistance(2,0),
                               pose_preprocessors=get_standard_pose_processors(zero_pad_shorter_poses=False)
                               )
    
    Ham2Pose_nDTW_MJE_Metric = DTWMetric(name="nDTW_MJE", 
    ...
    Ham2PoseAPEMetric = DistanceMetric(
        ...
        pose_preprocessors=get_standard_pose_processors(
            ...
            set_masked_values_to_origin=True),
    )
