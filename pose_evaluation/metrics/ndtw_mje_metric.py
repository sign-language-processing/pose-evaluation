...
from pose_evaluation.utils.pose_utils import PoseProcessor, pose_hide_low_conf, preprocess_pose
...
class DynamicTimeWarpingMetric(DistanceMetric):
    def __init__(self, spatial_distance_function_kind: Literal['euclidean'] | Literal['manhattan'] = "euclidean", preprocessors: None | List[PoseProcessor] = None, normalize_poses: bool = True, reduce_poses_to_common_points: bool = True, zero_pad_shorter_sequence: bool = True, remove_legs: bool = True, remove_world_landmarks: bool = True, conf_threshold_to_drop_points: None | float = None):
        super().__init__(spatial_distance_function_kind, preprocessors, normalize_poses, reduce_poses_to_common_points, zero_pad_shorter_sequence, remove_legs, remove_world_landmarks, conf_threshold_to_drop_points)
    

    def trajectory_pair_distance_function(self, hyp_trajectory, ref_trajectory) -> float:
        return fastdtw(hyp_trajectory, ref_trajectory, dist=self.point_pair_distance_function)
...
class NormalizedDynamicTimeWarpingMetricMeanJointErrorMetric(DynamicTimeWarpingMetric):
    def __init__(self, spatial_distance_function_kind: Literal['euclidean'] | Literal['manhattan'] = "euclidean", preprocessors: None | List[PoseProcessor] = None, normalize_poses: bool = True, reduce_poses_to_common_points: bool = True, zero_pad_shorter_sequence: bool = True, remove_legs: bool = True, remove_world_landmarks: bool = True, conf_threshold_to_drop_points: None | float = None):
        super().__init__(spatial_distance_function_kind, preprocessors, normalize_poses, reduce_poses_to_common_points, zero_pad_shorter_sequence, remove_legs, remove_world_landmarks, conf_threshold_to_drop_points)
