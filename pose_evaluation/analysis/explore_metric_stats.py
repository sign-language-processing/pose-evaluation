...
# --- Path input ---
project_folder = "/opt/home/cleong/projects/pose-evaluation"
csv_paths_default = [
    # f"{project_folder}/metric_results/4_22_2025_csvcount_17187_score_analysis_with_updated_MAP/stats_by_metric.csv",
    # f"{project_folder}/metric_results/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_round_2/4_23_2025_score_analysis_3300_trials/stats_by_metric.csv",
    # f"{project_folder}/metric_results_round_2/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_round_3/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_z_offsets_combined/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_round_4/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_embeddings/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_1_2_z_combined_818_metrics/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_round_2/score_analysis/stats_by_metric.csv",
    # f"{project_folder}/metric_results_round_4/score_analysis/stats_by_metric.csv",
    f"{project_folder}/metric_results_round_4/5_14_169_glosses_40680_metrics_score_analysis/stats_by_metric.csv",
    f"{project_folder}/metric_results_round_4/5_12_2025_score_analysis_288_metrics_169_glosses/stats_by_metric.csv",
    f"{project_folder}/metric_results_embeddings/5_2_2025_embedding_score_analysis_169_glosses/stats_by_metric.csv",
]
...
if csv_paths_input:
    ...
    st.write(f"Loaded {len(df)} metric stats, with {len(df[METRIC_COL].unique())} unique metric names")
    st.write(f"And {len(df[SHORT_COL].unique())} unique short names")
    ...
    # --- Plot ---
    plot_title = f"{sort_col.replace("_"," ").capitalize()} by Metric"
    ...
    if legend_title:
        plot_title = plot_title + f" and {legend_title}"

    if len(df) != metric_count:
        plot_title = plot_title + f" ({top_or_bottom} {len(df)} of {metric_count} metrics"
    else:
        plot_title = plot_title + f" ({metric_count} metrics"

    if trials_count is not None:
        plot_title = plot_title + f" over {trials_count:,} trials"

    if exclude:
        plot_title = plot_title + f", excl. '{exclude}'"

    if min_hyp > 0:
        plot_title = plot_title + f", excl. metrics w/ < {min_hyp:,} trials"

    plot_title = plot_title + ")"
    plot_title = st.text_input("Title for plot", value=plot_title)
